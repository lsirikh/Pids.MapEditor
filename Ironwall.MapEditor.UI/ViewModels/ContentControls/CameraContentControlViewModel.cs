...
{
    public class CameraContentControlViewModel
        : SymbolContentControlViewModel
    {
        #region - Ctors -
        public CameraContentControlViewModel(int id, IEventAggregator eventAggregator = null, EntityCollectionProvider<SymbolContentControlViewModel> provider = null) : base(id, eventAggregator, provider)
        {
        }

        public CameraContentControlViewModel(IEntityModel model, IEventAggregator eventAggregator, EntityCollectionProvider<SymbolContentControlViewModel> provider = null, MapProvider mapProvider = null) : base(model, eventAggregator, provider)
        {
            MapProvider = mapProvider;
        }

        public CameraContentControlViewModel(int id, string nameArea, int typeDevice, string nameDevice, int idController, int idSensor, int typeShape, double x1, double y1, double x2, double y2, double width, double height, double angle, int map, bool used, bool visibility, IEventAggregator eventAggregator = null, EntityCollectionProvider<SymbolContentControlViewModel> provider = null) : base(id, nameArea, typeDevice, nameDevice, idController, idSensor, typeShape, x1, y1, x2, y2, width, height, angle, map, used, visibility, eventAggregator, provider)
        {
        }
